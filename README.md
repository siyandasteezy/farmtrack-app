...
isibaya is hosted on **Netlify**; `isibaya.smartpick.co.za` is a subdomain of the
...
2. **Site settings → Environment variables:**
   ...
   - `APP_URL` — `https://isibaya.smartpick.co.za` (optional; redirects fall back
     to the request origin).
3. Deploy.

### 2. Custom domain (Netlify + Afrihost DNS)

1. Netlify → **Domain management → Add a domain** → `isibaya.smartpick.co.za`.
...
2. In **Afrihost** (the DNS host for `smartpick.co.za`) → your DNS zone → add a
   record for the `isibaya` subdomain:
   - **CNAME** `isibaya` → the `*.netlify.app` host Netlify gave you.
...
3. For live payments (`sk_live_…`), add **`isibaya.smartpick.co.za`** under Yoco's
   **Verified Domains** — live keys stay locked until a domain is verified.

> **Note:** isibaya has no database — after a Yoco-verified payment, access is
